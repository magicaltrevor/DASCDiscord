...
<head>
  ...
  <!-- Fonts: Dune-like feel (display) + modern readable body -->
  ...
  <style>
    :root{
      /* Desert night + sand gold palette */
      --bg:        #0b0e12;  /* page background */
      --surface:   #12161d;  /* card bg */
      --surface-2: #171c24;  /* input bg */
      --border:    #29303a;
      --text:      #e7e2d6;  /* primary text (warm off-white) */
      --muted:     #b9b2a3;  /* secondary text */
      --gold:      #c8a86a;  /* sand-gold accent */
      --gold-2:    #a8823e;  /* darker gold (hover/focus) */
      --warn:      #e0b156;
      --error:     #d46a6a;
    ...
    /* Base */
    html,body{
      background: radial-gradient(1200px 800px at 60% -10%, #121720 0%, #0b0e12 60%, #0b0e12 100%) fixed;
      color: var(--text);
    ...
    /* Add a sand-gold underline to selected headings */
    h2.section-title {
      position: relative;
      padding-bottom: .4rem;
    }
    h2.section-title::after {
      content: "";
      position: absolute;
      left: 0;                /* change to 50% + transform for centered */
      bottom: -0.2rem;
      width: 96px;            /* length of the stripe */
    ...
    /* Cards */
    .card{ border-image: linear-gradient(180deg, rgba(200,168,106,.35), rgba(200,168,106,.05)) 1; }

    /* Badges / hr */
    .badge{
      display:inline-block; margin-left:.5rem; padding:.2rem .55rem;
      border-radius: 999px; border:1px solid var(--gold);
      color: var(--gold); font-size:.8rem; vertical-align: middle;
    }
    hr{
      border: none; height: 1px;
      background: linear-gradient(90deg, rgba(200,168,106,.0), rgba(200,168,106,.6), rgba(200,168,106,.0));
      margin: 1rem 0;
    }
    ...
    input:focus, select:focus, textarea:focus{
      ...
      box-shadow: 0 0 0 3px rgba(200,168,106,.2);
    }
    ...
    button.warn{
      background: linear-gradient(180deg, #f0c06f, #b98024);
      border-color: #a36c1d;
      color: #1b140a;
    }
