...
def calculate_spice_distribution(
    ...
    dict
        ...
        ``melange_per_player``
            The amount of Spice Melange each player receives, rounded down to the
            nearest whole number. Any fractional remainder is not allocated.
    ...
    * 2 700 seconds → 200 melange
    ...
    # Distribute melange among players. The per-player amount is rounded
    # down to the nearest whole number because partial units cannot be
    # allocated. Any remainder remains unassigned.
    melange_per_player_exact = total_melange / players
    melange_per_player = math.floor(melange_per_player_exact)
...
def main() -> None:
    ...
    try:
        ...
        print(f"Spice Melange per player: {int(result['melange_per_player'])}")
